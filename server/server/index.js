...
const serverOrigins = ["http://localhost:3000", "http://localhost:3002", "http://localhost:3003", "http://localhost:3010"];
...
app.get('/test', async (req,res) => {
    const usernames = ['kenny', 'test','ress']
    let uname;
    let coll = [];
    for(var i = 0; i < usernames.length; i++){
         uname = usernames[i];
         const userss  = await users.findAll({where: {username: [usernames[i]]}, attributes: ['contacts']})
         userss.forEach(async(item) => {
            const test = item.dataValues.contacts
            if(coll.push(test)){
                console.log('hi')
            }
         })   
    }
    console.log(coll.reduce((sum,num) => sum + num))
    })

app.get('/test2', async (req,res) => {
        const username = 'kenny'
        const user = await users.findOne({where:{username:username}});
        const created = new Date(user.dataValues.createdAt + 'Z');
        const dateNow = new Date(Date.now()) + '+08:00';
         const date = String(created.getUTCDate());
         const month = String(created.getUTCMonth() + 1);
         const year = String(created.getUTCFullYear());
         console.log(`${date}/${month}/${year}`);
        res.json({created:created, datenow:dateNow})
        })
